...
# In this section we are going to demonstrate how to:
#
...
# ## Creating a ui.json
#
# The `ui.json` format is based on the [json](https://www.json.org/json-en.html)
# standard - a format widely used in programming. Python comes by default with
# a [json](https://docs.python.org/3/library/json.html) module to read and write to file.
# All we need is a way to create the structure that ANALYST understands.
# We can make use of the
# [geoh5py.ui_json](https://geoh5py.readthedocs.io/en/stable/content/api/geoh5py.ui_json.html) module to get started.

from geoh5py.ui_json import InputFile, constants, templates

# First, we need the base components must contain a few base parameters,
# such as a `title` and a `run_command`. A `constants.default_ui_json` is available to quickly get starget:

my_ui = constants.default_ui_json.copy()
my_ui["title"] = "Hello World!"
my_ui

# Note that the `run_command` is currently set to `None`, and therefore nothing
# will be executed by ANALYST. We will get to that part later.
#
...
# which writes it out as text file under the `assets` directory. From here you
# can drag & drop the file to the `Viewport` of ANALYST, which would render as:
#
# ![base_ui](./images/base_ui.png)
#
# This user-interface doesn't do much at the moment, so we can add a few additional forms.
# More parameters can be added by a simple `update` of the base dictionary.
# You can use any of the 8 `forms` currently supported:
#
